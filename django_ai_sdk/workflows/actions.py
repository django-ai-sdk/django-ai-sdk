...
class ThreadMessageAction:
    ...
    Costs an extra model call when the agent has `title_generation` on, since the
    thread is retitled from its content.
    """
...
async def _retitle(thread_id: str, content: str, context: ActionContext) -> None:
    ...
    This is a model call. Returns quietly when the agent is gone or opts out.
    """
...
def get_action_registry() -> dict[str, type[BaseAction]]:
    ...
    Read at call time, so a settings change needs no restart. A path that will not
    import is left out with a warning.
    """
    ...
    for key, path in extra.items():
        try:
            ...
            logger.warning(
                "AI_SDK_WORKFLOW_ACTIONS[%r] = %r could not be imported (%s); skipping.",
                key,
                path,
                exc,
            )
