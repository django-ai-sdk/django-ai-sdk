"""Startup diagnostics for automations.

All Warnings: a background job must never fail boot.
"""
...
def check_automations(app_configs: Any = None, **kwargs: Any) -> Sequence[Any]:
    ...
    for name, automation in automations.items():
        ...
        issues.extend(_check_workflow_input(name, automation))
...
def _check_workflow_input(name: str, automation: Any) -> list[Any]:
    ...
    definition = get_declared_workflows().get(automation.workflow)
    if definition is None:
        # Nothing to say beyond W003, or it is a database workflow.
        return []
