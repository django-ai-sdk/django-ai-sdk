...
A tick is safe to run on any number of hosts -- the claim is one conditional UPDATE
that advances the cursor and takes the lease together. Dispatch is at-least-once.
"""
...
async def claim(
    ...
    Predicate and write are one statement, so a second tick's UPDATE matches no rows.
    """
    schedule = automation.get_schedule()
...
async def release(state: AutomationState, *, succeeded_at: datetime | None = None) -> None:
    """Drop the lease this dispatch took, leaving one a later tick took alone.

    Predicated on the lease value, so a straggler cannot free a dispatch in flight.
    """
...
async def _enqueue_all(runs: list[AutomationRun]) -> None:
    ...
    Enqueued from on_commit, so a caller inside a transaction cannot hand a worker
    an id it would not find.
    """
    for run in runs:
...
async def run_now(name: str) -> list[AutomationRun]:
    ...
    Resolves the audience as a tick does, so this fans out to every subscriber, not
    to whoever asked.
    """
