"""How many runs one tick of an automation produces, and whose identity each carries.

An audience fans a single due automation out into a run per principal: `APP` is one
run with no principal, `SUBSCRIBED` is one per opted-in user. Two are shipped;
implement the `AudienceResolver` protocol for anything else.
"""
...
class Audience:
    """Namespace holding the two built-in resolvers, ready to assign to `audience`."""
