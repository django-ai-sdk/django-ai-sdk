...
def _croniter() -> Any:
    # Cron is the only thing in the SDK that needs croniter, so it ships in the
    # `automations` extra and is imported where a schedule is actually built.
    try:
        from croniter import croniter
    except ImportError as exc:  # pragma: no cover - depends on install extras
        raise ImproperlyConfigured(
            "Cron schedules need croniter. Install django-ai-sdk[automations]."
        ) from exc
    return croniter
