...
Whether it can *receive* the turn is the same kind of check. A workflow validates its inputs against its declared `input_fields` and drops what it does not declare, so an automation whose `input_name` names nothing produces a run that succeeded having asked the model nothing — no exception, no failed row, just an empty first turn. `W007` reports that pairing at startup for code-declared workflows; a database workflow is out of its reach, as with `W003`.
...
An audience resolving to `[None]` (as `Audience.APP` does) means one run with no principal. Anything downstream that needs a user — a hook delivering into a thread, a per-user integration credential, a non-public runtime agent — has none, so app-level automations pair with delivery that does not depend on one.
