...
**An `Audience.APP` run has no user, so it cannot deliver into a chat** — there is nobody to own the thread, and a hook that needs one logs and returns rather than failing the run. App-level work either delivers through a tool the agent holds, through a hook you register, or not at all: its result is recorded on the `AutomationRun` and readable from the admin and the API. If you want a message in someone's chat, you want `Audience.SUBSCRIBED`.
...
A workflow owns the *behaviour* — the steps, their system prompts, the hooks that deliver the result — and is declared in an app's `workflows.py` where every other caller can reach it. The automation owns the *occasion*: when it is due, whose identity it acts with, and the input that occasion starts from.
...
**A hook on the workflow.** A hook watches the run and delivers what a step produced — posting the result into a thread for the run's user, for instance. Delivery into a chat needs a user, so pair it with `Audience.SUBSCRIBED`; an `Audience.APP` run has nowhere to deliver and logs instead.

The SDK ships no delivery hooks: where a result belongs is the host's business, not the engine's. The demo registers a `thread_message` hook in `apps/agents/hooks.py`, which is the shape to copy.

**Register your own hook** when delivery must be deterministic rather than left to a model's choice to call a tool:

```python
AI_SDK_WORKFLOW_HOOKS = {"slack_post": "apps.alerts.hooks.SlackPostHook"}
```

A hook is a class with four optional callbacks — `on_run_start`, `on_step_start`, `on_step_end`, `on_run_end` — each taking the run's `WorkflowContext`. Named on the workflow it fires for every step; named on a step, for that step alone. A hook that raises is logged against its own name and never fails work that succeeded.
